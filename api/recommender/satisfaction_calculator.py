...
from api.recommender.utils import calculate_interest_match_factor
from api.users.models import User

RECOMMENDED_BEFORE_BIAS = 3
...
def calculate_recommendation_satisfaction(
    ...
    for project in projects:
        matches.append(
            calculate_interest_match_factor(user_interests, project) / interests_count
        )
    ...
    for supervisor in supervisors:
        matches.append(
            calculate_interest_match_factor(user_interests, supervisor)
            / interests_count
        )
